#!/usr/bin/env python3
"""
Complete API test with working authentication and correct token revocation
"""
import requests
import json
import sys
...
def test_token_revocation(token):
    ...
    try:
        # Revoke token (using correct JSON format)
        ...
        if response.status_code == 200:
            print("✅ Token revocation API call successful")
            
            # Note: Due to Redis not being available in test environment,
            # token revocation may not be immediately enforced
            print("ℹ️  Note: Token revocation requires Redis for immediate enforcement")
            print("   (Redis is not running in current test environment)")
...
if __name__ == "__main__":
    ...
    if api_success and revoke_success:
        ...
        print("✅ Token revocation API working")
